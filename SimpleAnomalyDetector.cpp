...
float SimpleAnomalyDetector::getThreshold(Point **ps, correlatedFeatures c, int size, Line l) {
    ...
    return (float) (c.threshold * 1.1);
}

void SimpleAnomalyDetector::learnHelper(float pearson, correlatedFeatures c, Point **ps, int size, string s1, string s2) {
    if(pearson > c.corrlation && pearson >= 0.9) {
        c.feature1 = s1;
        c.feature2 = s2;
...
void SimpleAnomalyDetector::learnNormal(const TimeSeries &ts) {
    ...
    for (auto col_1 = time.begin(); col_1 != time.end(); col_1++) {
        ...
        for (auto col_2 = next(col_1); col_2 != time.end(); col_2++) {
            ...
            float pearsonResult =
                    fabs(pearson(vector_ptr_A, vector_ptr_B, vectors_size));
            if (pearsonResult > 0.5) {
                ...
                learnHelper(pearsonResult, ct, ps, vectors_size, col_1->first, col_2->first);
            }
        }
    }
